...
var i=n.data.content,s=i.match(/%([^\s]+?)%/g);(null==s||"undefined"==typeof s)&&(s=[]),console.log(s)
for(var o=[],a=0,p=s.length;p>a;a++){var l=i.match(/%([^\s]+?)%/)[1]
l.match(/$if-/)||-1!==["snippet-id","snippet-name","user_login","user_email","user_firstname","user_lastname","user_pseudo"].indexOf(l)||o.push({type:"textbox",name:l,label:l,value:""})}t.windowManager.open({title:t.getLang("ithoughts_html_snippets_tinymce.edit_snippet")+'"'+n.data.title+'"',body:o,onsubmit:function(n){console.log(n.data)
var i=""
